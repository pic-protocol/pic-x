...
impl<'a> ServerContext<'a> {
    ...
    /// Adds the key ring this build signs with and publishes.
    ///
    /// Shared rather than borrowed, and it is the only collaborator here that is. Everything else on
    /// this context is *used* for the length of a call; a key ring is also *maintained* — rotated,
    /// retired, tidied — by work that outlives any one call and therefore cannot hold a borrow of
