...
mod tests {
    ...
    fn test_a_subject_entry_does_not_match_a_common_name_entry() {
        let by_subject = [AllowedPeer::Subject("CN=local-operator,O=PIC-X".to_owned())];
    ...
    fn test_a_peer_falls_back_to_something_a_human_can_read() {
        ...
        let no_name = PeerIdentity::new("O=PIC-X", None, FINGERPRINT, "01");
        assert_eq!(no_name.label(), "O=PIC-X");
