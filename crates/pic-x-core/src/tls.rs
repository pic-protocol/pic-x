...
mod tests {
    ...
    fn test_resolving_moves_every_path_into_the_volume_and_keeps_the_policy() {
        let resolved = TlsSettings::new("server.pem", "server.key")
            ...
            .resolved_in(Path::new("/var/lib/pic-x"));

        assert_eq!(
            resolved.certificate(),
            Path::new("/var/lib/pic-x/server.pem")
        );
        assert_eq!(resolved.crl(), Some(Path::new("/var/lib/pic-x/ca.crl")));
        assert_eq!(resolved.reload(), Some(Duration::from_secs(30)));
    }

    #[test]
    fn test_an_absolute_path_is_left_where_a_deployment_put_it() {
        let resolved = TlsSettings::new("/etc/pic-x/server.pem", "server.key")
            .resolved_in(Path::new("/var/lib/pic-x"));

        assert_eq!(resolved.certificate(), Path::new("/etc/pic-x/server.pem"));
        assert_eq!(resolved.key(), Path::new("/var/lib/pic-x/server.key"));
    }
}
