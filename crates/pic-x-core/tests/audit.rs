...
use anyhow::anyhow;
use pic_x_core::AuditError;
use std::sync::Mutex;

use pic_x_core::{AuditEvent, AuditSink, BoxFuture, Pseudonymizer, Sensitivity, Subject};
...
fn test_an_event_carries_what_happened_and_to_what() {
    let event = AuditEvent::system("server.start", "pic-x");

    assert_eq!(event.action(), "server.start");
    assert_eq!(event.subject(), Subject::System("pic-x"));
    assert_eq!(event.subject().value(), Some("pic-x"));
}
...
fn test_a_principal_is_personal_data_and_never_renders_readable() {
    ...
    assert_eq!(subject.to_string(), pic_x_core::redact::MASK);
...
fn test_without_a_policy_a_principal_renders_masked() {
    ...
    assert_eq!(subject.render(None), pic_x_core::redact::MASK);
}
...
async fn test_the_contract_is_implementable_from_outside_and_usable_as_a_trait_object() {
    ...
    sink.record(&AuditEvent::system("server.start", "pic-x"), None)
        .await
