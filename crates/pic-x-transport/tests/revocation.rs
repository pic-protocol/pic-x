...
use pic_x_transport::{Surface, load_certificates, load_key, server_config};
...
impl Authority {
    fn new(name: &str) -> Self {
        let directory = std::env::temp_dir().join(format!("pic-x-crl-{name}"));
...
fn identifying_router() -> Router {
    ...
    Router::new().route(
        "/",
        get(
            |identity: Option<Extension<Arc<pic_x_core::PeerIdentity>>>| async move {
                match identity {
                    Some(Extension(peer)) => {
...
async fn test_the_certificate_the_client_presented_reaches_the_handler() {
    ...
    let presented = pic_x_transport::digest(
