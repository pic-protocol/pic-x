...
use pic_x_core::metrics::{Metric, Sample};
use pic_x_core::{Health, Metrics};
...
const UP: Metric = Metric::gauge("picx_up", "Whether the process reports itself live.");
...
async fn metrics(State(reported): State<Reported>) -> (StatusCode, String) {
    let mut samples = vec![
        Sample {
            metric: UP,
            labels: Vec::new(),
            reading: pic_x_core::Reading::Value(f64::from(u8::from(reported.health.is_live()))),
        },
        Sample {
            metric: READY,
            labels: Vec::new(),
            reading: pic_x_core::Reading::Value(f64::from(u8::from(reported.health.is_ready()))),
        },
    ];
