...
use pic_x_core::metrics::{Kind, Reading, Sample};
...
mod tests {
    ...
    fn test_a_counter_is_described_once_and_then_listed() {
        let requests = Metric::counter("picx_requests_total", "Requests served.");
        let text = render(&[
        ...
        assert_eq!(
            text,
            "# HELP picx_requests_total Requests served.\n\
             # TYPE picx_requests_total counter\n\
             picx_requests_total{status=\"2xx\"} 7\n\
             picx_requests_total{status=\"5xx\"} 1\n"
        );
    }

    #[test]
    fn test_a_metric_with_no_labels_carries_no_braces() {
        let up = Metric::gauge("picx_up", "Whether the process reports itself live.");
        let text = render(&[sample(up, &[], Reading::Value(1.0))]);

        assert!(text.ends_with("picx_up 1\n"), "{text}");
    }

    #[test]
    fn test_a_histogram_ends_at_infinity_and_carries_its_totals() {
        let latency = Metric::histogram("picx_request_seconds", "How long requests took.", SECONDS);
        ...
        assert!(text.contains("# TYPE picx_request_seconds histogram\n"));
        assert!(text.contains("picx_request_seconds_bucket{le=\"0.001\"} 0\n"));
        assert!(text.contains("picx_request_seconds_bucket{le=\"0.005\"} 1\n"));
        assert!(
            text.contains("picx_request_seconds_bucket{le=\"+Inf\"} 3\n"),
            "the observation above the last boundary was lost: {text}"
        );
        assert!(text.contains("picx_request_seconds_sum 120.403\n"));
        assert!(text.contains("picx_request_seconds_count 3\n"));
    }
    ...
    fn test_a_label_value_cannot_break_out_of_the_line_it_is_on() {
        ...
        let text = render(&[sample(
            requests,
            &[("outcome", "x\" } 999\npicx_requests_total{outcome=\"forged")],
            Reading::Value(1.0),
        )]);
    ...
    fn test_help_text_cannot_break_out_either() {
        let odd = Metric::counter("picx_odd_total", "First line.\nsecond line.");
