...
use pic_x_core::{Secret, SecretError, SecretRef, SecretStore};
use pic_x_std::secrets::{DirectorySecretStore, EnvironmentSecretStore, InMemorySecretStore};
...
fn directory(name: &str) -> PathBuf {
    let path = std::env::temp_dir().join(format!("pic-x-secrets-{name}"));
...
fn test_a_secret_from_the_environment_resolves_by_its_variable() {
    ...
    unsafe { std::env::set_var("PIC_X_TEST_AUDIT_PSEUDONYM", "from-the-environment") };

    let resolved =
        EnvironmentSecretStore::new("PIC_X_TEST").resolve(&SecretRef::new("audit-pseudonym"));

    unsafe { std::env::remove_var("PIC_X_TEST_AUDIT_PSEUDONYM") };
...
fn test_an_unset_variable_is_an_absence_not_a_failure() {
    let error = failure(
        EnvironmentSecretStore::new("PIC_X_TEST_NOTHING").resolve(&SecretRef::new("nowhere")),
        "nothing is set",
    );
...
fn test_every_store_names_itself() {
    ...
    assert_eq!(EnvironmentSecretStore::new("PIC_X").name(), "environment");
