...
//! timestamp inside each record that a human can read. Both are calendar arithmetic on UTC, which is
//! a closed-form problem with a published solution — Howard Hinnant's `civil_from_days`, exact for
//! every date the type can hold and with no table to get wrong.
//!
