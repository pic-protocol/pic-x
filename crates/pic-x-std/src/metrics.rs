...
use pic_x_core::metrics::{Kind, Label, Metric, Reading, Recorder, Sample};
...
mod tests {
    ...
    use pic_x_core::metrics::SECONDS;

    const REQUESTS: Metric = Metric::counter("picx_requests_total", "Requests served.");
    const CONNECTIONS: Metric = Metric::gauge("picx_connections", "Connections held.");
    const LATENCY: Metric =
        Metric::histogram("picx_request_seconds", "How long requests took.", SECONDS);
    ...
    fn test_a_counter_accumulates_and_a_gauge_replaces() {
        ...
        assert_eq!(only(&registry, "picx_requests_total"), Reading::Value(5.0));
        assert_eq!(only(&registry, "picx_connections"), Reading::Value(12.0));
    }
    ...
    fn test_a_histogram_counts_every_bucket_an_observation_falls_within() {
        ...
        let Reading::Distribution {
            ...
            count,
            sum,
        } = only(&registry, "picx_request_seconds")
        else {
    ...
    fn test_labels_given_in_any_order_are_one_series() {
        ...
        assert_eq!(only(&registry, "picx_requests_total"), Reading::Value(2.0));
    }
