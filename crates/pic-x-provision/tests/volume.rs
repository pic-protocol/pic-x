...
use pic_x_provision::{Volume, prepare};

/// A volume location nothing else is using.
fn volume(name: &str) -> PathBuf {
    let path = std::env::temp_dir().join(format!("pic-x-volume-{name}"));
...
fn generating(root: &Path, extra: &[(&str, &str)]) -> Config {
    let mut pairs = vec![
        ("PIC_X_WORKING_DIR", root.to_str().expect("a UTF-8 path")),
        ("PIC_X_AUTOGENERATE", "true"),
        ("PIC_X_SECRETS_PROVIDER", "directory"),
        ("PIC_X_AUDIT_PSEUDONYM_ENABLED", "true"),
        ("PIC_X_AUDIT_PSEUDONYM_KEY_REF", "audit-pseudonym"),
    ];
...
fn test_the_volume_is_created_even_when_nothing_may_be_generated() {
    ...
    let config = config(&[("PIC_X_WORKING_DIR", root.to_str().expect("a UTF-8 path"))]);
...
fn test_a_secret_is_generated_when_generation_is_allowed() {
    ...
    let secret = root.join("secrets").join("audit-pseudonym");
...
fn test_two_runs_never_produce_the_same_secret() {
    ...
    assert_ne!(
        fs::read_to_string(first.join("secrets/audit-pseudonym")).expect("the first secret"),
        fs::read_to_string(second.join("secrets/audit-pseudonym")).expect("the second secret"),
...
fn test_what_is_already_there_is_never_overwritten() {
    ...
    fs::create_dir_all(root.join("secrets")).expect("creating the directory");
    fs::write(
        root.join("secrets/audit-pseudonym"),
    ...
    assert_eq!(
        fs::read_to_string(root.join("secrets/audit-pseudonym")).expect("the secret is there"),
        "supplied-by-somebody-else",
...
fn test_nothing_is_generated_without_permission() {
    ...
    assert!(
        !root.join("secrets/audit-pseudonym").exists(),
...
fn test_generated_material_is_readable_by_nobody_else() {
    ...
    for private in ["secrets/audit-pseudonym", "tls/server.key", "tls/ca.key"] {
