...
A realm is an **issuer**: its own signing keys, its own audit trail, its own pseudonymisation. One
deployment can host many. The server that hosts them is not one of them — it signs the system trail
and lists the realms, and it issues nothing.
...
| signing keys | `keys/` — signs the **system trail** | `realms/<name>/keys/` — signs its trail and its tokens |
| audit trail | `audit/` — server and admin events | `realms/<name>/audit/` — that realm's events |
| pseudonymisation key | `secrets/audit-pseudonym` | `realms/<name>/secrets/audit-pseudonym` — a *distinct* key |
...
/.well-known/jwks.json                             the server's key (verifies the system trail's seals)
/realms/<name>/.well-known/pic-x-configuration     that realm's issuer discovery
/realms/<name>/.well-known/jwks.json               that realm's keys
```
...
`realms/<name>/secrets/` — isolated, and autogenerated in development. With `provider: environment`,
it resolves from an environment variable under a per-realm prefix that defaults to
`<server-prefix>_<REALM>` (e.g. `PIC_X_SECRET_ACME_AUDIT_PSEUDONYM`), so two realms cannot collide.
A realm's key lifecycle is held to the same overlap rules as the server's, per realm — an override
...
└── realms/
    ├── acme/{keys,audit,secrets}/     acme's own, isolated
    └── beta/{keys,audit,secrets}/     beta's own, isolated
```
