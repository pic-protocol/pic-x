#!/usr/bin/env bash
#
# Fails when `pic-x-core` grows a dependency outside its allowlist.
#
# Every crate in the workspace — and every crate a downstream build writes to implement one of these
# contracts — depends on `pic-x-core`. Whatever lands in its dependency list lands in all of them, and
...
MANIFEST="${ROOT}/crates/pic-x-core/Cargo.toml"
...
if [ -n "${violations}" ]; then
    printf 'pic-x-core declares dependencies outside its allowlist:\n%s' "${violations}" >&2
    printf 'error: allowed are %s\n' "${ALLOWED[*]}" >&2
    exit 1
fi

printf 'ok: pic-x-core depends only on %s\n' "${ALLOWED[*]}"
